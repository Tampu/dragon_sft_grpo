...
import transformers
from internvl_lora_checkpoint_io import save_lora_checkpoint
...
from internvl.model.internvl_chat import (InternVisionConfig,
                                          ...
                                          InternVLChatConfig,
                                          InternVLChatModel)
from internvl.model.internvl_chat.modeling_intern_vit import has_flash_attn
...
class DataTrainingArguments:
    ...
    checkpoint_steps: str = field(
        default='',
        metadata={'help': 'Comma-separated global steps to save an adapter-only checkpoint at, '
                           'e.g. "100,250,500,750,1000". Independent of save_strategy (which stays '
                           '"no" to avoid the ~16GB full-merge save path) -- uses save_lora_checkpoint.'},
    )
    eval_after_checkpoint: bool = field(
        default=False,
        metadata={'help': 'After each checkpoint_steps save, block training and run '
                           'run_dragon6_exp1_eval.py against it (writes to <checkpoint>/eval).'},
    )
...
def get_first_system_message(meta_path: str) -> str:
    """Read the system-turn text actually present in this run's training
    data, so config.system_message (used by model.chat() at inference) can
    never desync from whatever converter version built the data -- see the
    call site for why this replaced a hardcoded per-converter constant."""
    meta = json.loads(Path(meta_path).read_text())
    first_dataset = next(iter(meta.values()))
    annotation_path = Path(first_dataset['annotation'])
    if not annotation_path.is_absolute():
        annotation_path = Path(meta_path).resolve().parents[1] / annotation_path
    with annotation_path.open('r', encoding='utf-8') as f:
        first_record = json.loads(f.readline())
    conversations = first_record['conversations']
    if conversations[0]['from'] != 'system':
        raise ValueError(
            f"Expected conversations[0]['from'] == 'system' in {annotation_path}, "
            f"got {conversations[0]['from']!r}. Training data must carry an explicit "
            f"system turn for config.system_message to be derived correctly."
        )
    return conversations[0]['value']


def run_holdout_eval_subprocess(checkpoint_dir, hf_cache_dir, label):
    """Blocking call to run_dragon6_exp1_eval.py against one checkpoint.
    Training (still resident on the same GPU) is paused for the duration;
    a failure here is logged but never allowed to take down the training run.
    """
    root = Path(__file__).resolve().parents[1]
    checkpoint_dir = Path(checkpoint_dir)
    eval_cmd = [
        sys.executable, str(root / 'scripts' / 'run_dragon6_exp1_eval.py'),
        '--checkpoint-dir', str(checkpoint_dir),
        '--output-dir', str(checkpoint_dir / 'eval'),
        '--device', 'cuda:0',
    ]
    if hf_cache_dir:
        eval_cmd += ['--hf-cache-dir', hf_cache_dir]
    logger.info(f'[{label}] running eval: {" ".join(eval_cmd)}')
    result = subprocess.run(eval_cmd, cwd=str(root))
    if result.returncode != 0:
        logger.warning(f'[{label}] eval failed (exit {result.returncode}) -- continuing regardless.')


class StepCheckpointCallback(TrainerCallback):
    """Saves adapter-only checkpoints at specific global steps (independent of
    save_strategy, which stays "no" here to avoid the ~16GB full-merge save
    path -- see save_lora_checkpoint's docstring), optionally running holdout
    eval against each one before training resumes.
    """

    def __init__(self, checkpoint_steps, output_dir, model_args, eval_after_checkpoint, hf_cache_dir):
        self.checkpoint_steps = set(checkpoint_steps)
        self.output_dir = Path(output_dir)
        self.model_args = model_args
        self.eval_after_checkpoint = eval_after_checkpoint
        self.hf_cache_dir = hf_cache_dir

    def on_step_end(self, args, state, control, **kwargs):
        if state.global_step not in self.checkpoint_steps or not state.is_world_process_zero:
            return control
        model = kwargs['model']
        ...
        model.eval()
        try:
            save_lora_checkpoint(model, tokenizer, str(ckpt_dir), self.model_args)
        finally:
            if was_training:
                model.train()
        logger.info(f'[StepCheckpointCallback] saved step {state.global_step} -> {ckpt_dir}')

        if self.eval_after_checkpoint:
            run_holdout_eval_subprocess(ckpt_dir, self.hf_cache_dir, f'StepCheckpointCallback step={state.global_step}')
        return control
...
def main():
    ...
    if not hasattr(training_args, 'group_by_length'):
        # Removed from transformers' TrainingArguments in newer versions; InternVL's
        # vendored train_sampler_patch.py / train_dataloader_patch.py still read it
        # directly off training_args, so restore it as a real attribute.
    ...
    if model_args.model_name_or_path is not None:
        ...
        # Must match the system turn actually present in this run's training
        # data, so model.chat() at inference time (which reads
        # config.system_message from the saved checkpoint) sees the same
        # task framing training did. Read from data rather than hardcoding
        # one converter's system prompt constant -- multiple converter
        # versions exist now (v2 in dragon_grounding_convert_core, v3 in
        # sft_v3_patches) and a stale import here would silently desync
        # training from inference for whichever version isn't imported.
        config.system_message = get_first_system_message(data_args.meta_path)
        llm_attn_impl = 'flash_attention_2' if has_flash_attn else 'eager'
        if not has_flash_attn:
            logger.info('flash_attn package not installed, falling back to eager attention')
        if config.llm_config.model_type == 'internlm2':
            config.llm_config.attn_implementation = llm_attn_impl  # for InternLM
            logger.info(f'Using {llm_attn_impl} for InternLM')
        else:
            config.llm_config._attn_implementation = llm_attn_impl  # for LLaMA
            logger.info(f'Using {llm_attn_impl} for LLaMA')
        config.vision_config.use_flash_attn = has_flash_attn
        ...
        if llm_config.model_type == 'internlm2':
            ...
            llm_config.attn_implementation = llm_attn_impl  # for InternLM
            logger.info(f'Using {llm_attn_impl} for InternLM')
        else:
            model_type = AutoModelForCausalLM
            llm_config._attn_implementation = llm_attn_impl  # for LLaMA
            logger.info(f'Using {llm_attn_impl} for LLaMA')
    ...
    train_dataset = build_datasets(
        data_args, tokenizer, tcs_loader, model, group_by_length=getattr(training_args, 'group_by_length', False),
    ...
    callbacks = []
    if data_args.checkpoint_steps.strip():
        checkpoint_steps = [int(s) for s in data_args.checkpoint_steps.split(',') if s.strip()]
        callbacks.append(
            StepCheckpointCallback(
                checkpoint_steps=checkpoint_steps,
                output_dir=training_args.output_dir,
                model_args=model_args,
                eval_after_checkpoint=data_args.eval_after_checkpoint,
                hf_cache_dir=os.environ.get('HF_HOME'),
            )
        )
        logger.info(f'StepCheckpointCallback active for steps: {checkpoint_steps}, '
                    f'eval_after_checkpoint={data_args.eval_after_checkpoint}')
    ...
    if training_args.do_train:
        ...
        if trainer.is_world_process_zero():
            save_lora_checkpoint(model, tokenizer, training_args.output_dir, model_args)
            if data_args.eval_after_checkpoint:
                run_holdout_eval_subprocess(
                    training_args.output_dir, os.environ.get('HF_HOME'), 'final checkpoint'
                )
